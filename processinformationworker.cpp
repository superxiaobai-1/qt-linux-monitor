...
processInformationWorker::processInformationWorker(QObject *parent,
  ...
  connect(this, SIGNAL(updateLoadAverage(QString)), loadAverage,
  ...
  connect(actionProperties, SIGNAL(triggered(bool)), this,
          SLOT(showProcessProperties()));
  connect(processesTable, SIGNAL(doubleClicked(QModelIndex)), this,
          SLOT(showProcessProperties()));
  ...
  connect(filterCheckbox, SIGNAL(toggled(bool)), this,
          SLOT(filterCheckboxToggled(bool)));
  searchField = mainTabs->findChild<QLineEdit *>("processesSearchField");

  connect(searchField, SIGNAL(textChanged(QString)), this,
          SLOT(filterProcesses(QString)));
  connect(this, SIGNAL(signalFilterProcesses(QString)), this,
          SLOT(filterProcesses(QString)));
  // set a function to run when a row is selected
  connect(processesTable->selectionModel(),
          SIGNAL(currentRowChanged(QModelIndex, QModelIndex)), this,
          SLOT(changeCurrentTableRowSelection(QModelIndex)));
  ...
  filterCheckbox->setChecked(
      this->settings->value("processesFilterCheckbox", true).toBool());
}
...
void processInformationWorker::showProcessProperties() {
  processPropertiesDialogue *properties = new processPropertiesDialogue(
      (QWidget *)mainWindow, selectedRowInfoID, settings);
  properties->show();
  properties->exec();
}
...
/**
 * @brief processInformationWorker::changeCurrentTableRowSelection Select a row
 * in the table of processes and remember its position
 * @param current The selection model for the current selection
 */
void processInformationWorker::changeCurrentTableRowSelection(
    QModelIndex current) {
  // remember the tid so we can get the correct row again when we update the
  // table
  selectedRowInfoID = processesTable->item(current.row(), 3)->text().toInt();
}

/**
 * @brief processInformationWorker::createProcessesView construct the process
 * table with properly resized row heights
 */
void processInformationWorker::createProcessesView() {
  ...
  processesTable->setHorizontalHeaderLabels(
      QString("Process Name;User;% CPU;PID;Memory;").split(";"));
...
/**
 * @brief processInformationWorker::handleProcessStop Function to run when the
 * user selects stop as an option for a process
 */
void processInformationWorker::handleProcessStop() {
  ...
  std::string info = "Are you sure you want to stop " +
                     processesTable->item(row, 0)->text().toStdString();
  reply = QMessageBox::question(processesTable, "Info",
    ...
    if (kill(pid, SIGTERM) != 0) {
      if (errno == EPERM) {
        ...
        QMessageBox::information(processesTable, "Error",
                                 "Could not send signal, permission denied!",
                                 QMessageBox::Ok);
      }
    }
  }
...
/**
 * @brief processInformationWorker::handleProcessKill Function to run when the
 * user selects stop as an option for a process
 */
void processInformationWorker::handleProcessKill() {
  ...
  QMessageBox::StandardButton reply;
  std::string info = "Are you sure you want to kill " +
                     processesTable->item(row, 0)->text().toStdString();
  reply = QMessageBox::question(processesTable, "Info",
                                QString::fromStdString(info),
                                QMessageBox::Yes | QMessageBox::No);
...
/**
 * @brief processInformationWorker::shouldHideProcess Return if the current row
 * should be hidden when updating the table based on if the user has selected to
 * only show processes that they own
 * @param pid The pid of the process
 * @return True if the process is not owned by the user and they only want to
 * show their processes
 */
...
/**
 * @brief processInformationWorker::filterProcesses Hide all processes which do
...
void processInformationWorker::filterProcesses(QString filter) {
  ...
  if (!processesTable) {
    // qDebug() << "Error: processesTable is null.";
    return;
  }

  for (int i = 0; i < processesTable->rowCount(); i++) {
    if (processesTable->item(i, 1)) {
      if (!shouldHideProcess(
              getpwnam(processesTable->item(i, 1)->text().toStdString().c_str())
                  ->pw_uid)) {
        ...
        if (name && pid) {
          ...
          bool cmdLineContains =
              getProcessCmdline(pid->text().toInt()).toLower().contains(filter);
          processesTable->setRowHidden(i, (!nameContains && !cmdLineContains));
        } else {
          //   qDebug() << "Error: Table item is null.";
        }
      }
    } else {
      //   qDebug() << "Error: Table item is null.";
...
void processInformationWorker::updateTable() {
  ...
  if (prevProcs.size() > 0) {
    ...
    for (auto &newItr : processes) {
      ...
      newItr.second.pcpu = (unsigned int)calculateCPUPercentage(
          &prevItr, &newItr.second, totalCpuTime);
    }
  }
  ...
  for (auto &i : processes) {
    ...
    processNameTableItem->setIcon(
        getProcessIconFromName(processName, processIconCache));
    ...
    QString cpu = QString::number(
        settings->value("divide process cpu by cpu count", false).toBool()
            ? p->pcpu / smp_num_cpus
            : p->pcpu);
    ...
    memoryConverter memory = memoryConverter(
        (p->resident - p->share) * sysconf(_SC_PAGESIZE), memoryUnit::b,
        settings->value("unit prefix standards", JEDEC)
            .toString()
            .toStdString());
...
void processInformationWorker::loop() {
  ...
  double load[3];
  getloadavg(load, 3);
  // work out how overloaded the system was in the last minute based on CPU
  // alone yes, linux uses more than just CPU in the load calculations
  double overload = load[0] - smp_num_cpus;
  QString avg =
      "Load averages for the last 1, 5, 15 minutes: " +
      QString::number(load[0]) + ", " + QString::number(load[1]) + ", " +
      QString::number(load[2]) +
      (overload < 0 ? ""
                    : "\nOverloaded by " + QString::number(overload * 100) +
                          "% in the last minute!");
